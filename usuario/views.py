...
class RegistroUsuario(CreateView):
    ...
    def form_valid(self, form):
        ...
        contra = usuario.first_name[0].upper(
        )+str(usuario.identificacion)+usuario.last_name[0].upper()
        ...
        if (usuario.rol == 'director'):
            grupo_director, grupo_director_creado = Group.objects.get_or_create(
                name='director')
            grupo_director.user_set.add(self.object)
        elif (usuario.rol == 'investigador'):
            grupo_investigador, grupo_investigador_creado = Group.objects.get_or_create(
                name='investigador')
...
def Login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(
                request, 'El usuario o contraseña no coinciden o no existen')
            return redirect('usuario:login')
    else:
        # Si el usuario esta autenticado entonces nos direcciona a la url establecida en success_url
        if request.user.is_authenticated:
